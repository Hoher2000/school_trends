...
ЖЁСТКОЕ ПРАВИЛО: Если в заголовке или описании встречаются слова "Minecraft", "челлендж", "TikTok", "гаджет", "игра", "школьник", "roblox", "brawl stars", "аниме", "мем", "Наруто", "атака титанов" – новость АВТОМАТИЧЕСКИ ПОДХОДЯЩАЯ. Никогда не ставь skip для таких новостей. Это приказ.

Если новость НЕ подходит, верни СТРОГО {"skip":true} (один объект, без дополнительных полей).
Если новость ПОДХОДИТ, создай сценарий для вертикального видео длительностью 30 секунд.
...
НИКОГДА не используй двойные кавычки внутри full_text и subtitles. Если нужна цитата — используй «ёлочки» или одинарные кавычки.

ЗАПРЕЩЕНО использовать одни и те же приветствия и прощания. Каждый раз выбирай новый вариант из списка ниже, не повторяйся.
...
Верни СТРОГО ОДИН JSON-объект, без markdown и лишнего текста.
...
// GenerateScriptForced создаёт сценарий без проверки skip (новость заведомо подходит).
func (g *OpenRouterGenerator) GenerateScriptForced(title, description string) (string, error) {
	systemPrompt := `Ты — сценарист детского канала. Создай позитивный сценарий для вертикального видео (Shorts) длительностью 30 секунд на русском языке.
Разбей на 6 коротких предложений для субтитров.
Используй разные приветствия и концовки.
НИКОГДА не используй двойные кавычки внутри текста. Заменяй их на «ёлочки» или одинарные кавычки.
Верни СТРОГО JSON: {"full_text":"...","subtitles":["...","..."]}`
	userPrompt := fmt.Sprintf("Заголовок: %s\nОписание: %s", title, description)
	return g.callAPI(systemPrompt, userPrompt, 800)
}
